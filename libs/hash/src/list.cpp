...
List::__remove_internal(_list_entry_t* __val)
{
    ...
    _list_entry_t *_del_entry = __val;
...
List::get_head()
{
    return (m_head) ? m_head->val : nullptr;
}

void*
List::get_tail()
{
    return (m_tail) ? m_tail->val : nullptr;
}

bool
List::remove_head()
{
    return remove_entry(m_head);
}

bool
List::remove_tail()
{
    return remove_entry(m_tail);
}
List::~List()
{
    delete m_tail;
